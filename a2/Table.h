...
public:
    ...
    // Returns ColumnNames
    ColumnNames get_name();

    // Returns RowSet
    RowSet get_row();

    // Adds all rows in a table
    void table_onion(RowSet);

    // Renames columns in a table
    void table_rename(const NameMap &);

    // Selects certain rows to return based on predicate
    void table_select(RowSet, RowPredicate);
    ...
    /*
    // Helper function to get position of a column given its name
   const int col_pos(const string &name);
   */
