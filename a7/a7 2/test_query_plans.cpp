        ...
        sort ( 
            unique(
                project (
                    nested_loops_join ( 
                                        nested_loops_join (
                                                    select(table_scan(user),q2_predicate), {0},
                                                    table_scan(routing), {0}),
                                        {4}, table_scan(message), {0}),
                    {5})),
            {0});
    /*
    print_iterator ("q2_table", 
        sort (
            unique(
                project (
                    nested_loops_join ( 
     ...
     sort ( 
            unique(
                project (
                    nested_loops_join ( 
                                        nested_loops_join (
                                                    index_scan(username_index, &username), {0},
                                                    table_scan(routing), {0}),
                                        {4}, table_scan(message), {0}),
                    {5})),
            {0});

    // print_iterator ("q2_index", index_scan(username_index, &username));
    /*
    index_scan(username_index, &x)
    sort (
        project (    
                nested_loops_join (
                                ...
                                nested_loops_join (
                                            table_scan(routing), {0},
                                            index_scan(username_index, &username), {0}), {2}),
                {1}),
        {0}));
    */
    CHECK(match(c2, q2));
...
static bool q3_predicate(const Row *row)
{
    ...
    return row->at(2).substr(5, 5) == row->at(6).substr(5, 5);
}
    ...
    project (
        select (
            nested_loops_join(table_scan(message), {0},
                nested_loops_join(table_scan(routing), {1}, table_scan(user), {0}), {2}),
            q3_predicate),
        {3});
        //IMPLEMENT_ME;
    ...
    project (
        nested_loops_join(
            nested_loops_join(
                nested_loops_join (
                                project (
                                    index_scan(username_index, &unguiferous), 
                                    {0}), {0},
                                table_scan(routing), {0}),{1},
                project (
                    index_scan(username_index, &froglet), 
                    {0}),{0}), {2},
            table_scan(message), {0}),
        {3});
    /*
